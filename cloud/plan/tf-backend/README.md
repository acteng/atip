...
1. Fetch the previous Terraform state from Bitwarden, if any:
   ...
   ```bash
   bw get notes "plan-tf-backend-state" > terraform.tfstate
   ```
...
1. Store the new Terraform state in Bitwarden as "plan-tf-backend-state"
...
1. Delete the Terraform state "plan-tf-backend-state" from Bitwarden 
